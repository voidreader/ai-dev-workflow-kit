...
### 1-3. 대상 인스턴스 확인 (인스턴스 수와 무관하게 필수)

MCP 서버는 하나인데 로컬 Unity 프로젝트 여러 개가 그 서버를 공유한다. 인스턴스 이름이 전부 `Unity@<해시>` 형태라 **이름만으로는 어느 프로젝트인지 알 수 없다.** 자동 호출 전에 반드시 다음을 수행한다.

1. `mcpforunity://instances`로 인스턴스 목록을 받는다.
2. 각 인스턴스의 `mcpforunity://editor/state`에서 `data.editor.active_scene.path`를 읽어 현재 작업 디렉터리의 프로젝트 경로와 대조한다.
3. 대상이 확정되면 `set_active_instance`로 고정하거나, 호출마다 `unity_instance="<해시 앞부분>"`을 명시한다. 여러 세션을 동시에 돌릴 때는 호출 단위 지정이 안전하다.
4. 현재 프로젝트의 인스턴스가 목록에 없으면 **다른 프로젝트 인스턴스로 대신 실행하지 않는다.** 미연결로 보고 2-B로 분기한다.

> **인스턴스가 하나뿐이어도 건너뛰지 않는다.** 2개 이상이면 대상 미지정 시 서버가 에러를 내지만, **1개뿐일 때는 에러 없이 그 인스턴스로 조용히 라우팅된다.** 확인 없이 호출해 다른 프로젝트의 테스트 결과를 현재 프로젝트 결과로 보고한 사고가 실제로 있었다(2026-09-13). 인스턴스가 하나뿐이라는 사실은 그것이 올바른 대상이라는 근거가 되지 않는다.
